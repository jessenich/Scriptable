...
namespace Scriptable {
    internal static class ProcessHelper {
        public const string ProcessNotAccessibleWithDisposeOnExitEnabled = "Process can only be accessed when the command is not set to dispose on exit. This is to prevent non-deterministic code which may access the process before or after it exits";
        ...
        public static Task<int> CreateProcessTask(
            ...
            if (timeout != Timeout.InfiniteTimeSpan) {
                ...
                disposables.Add(timeoutSource.Token.Register(() => {
                    ...
                    if (Interlocked.CompareExchange(ref resultObject, timeoutException, null) == null) TryKillProcess(process); // if timeout wins the race, kill the process
                }));
