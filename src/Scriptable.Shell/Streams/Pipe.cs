        ...
        private void UpdateSignalsNoLock() {
            ...
            if (this._spaceAvailableSignal != null)
                switch (this._spaceAvailableSignal.CurrentCount) {
                    case 0:
                        if (this._readerClosed || this.GetSpaceAvailableNoLock() > 0) this._spaceAvailableSignal.Release();
                        break;
                    case 1:
                        if (!this._readerClosed && this.GetSpaceAvailableNoLock() == 0) this._spaceAvailableSignal.Wait();
                        break;
                    default:
                        throw new InvalidOperationException("Should never get here");
                }
