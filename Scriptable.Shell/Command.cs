...
using Scriptable.Shell.Signals;
    ...
    public abstract class Command : IDisposable {
        ...
        public Task<bool> TrySignalAsync(CommandSignal signal) {
            ...
            async Task<bool> TrySignalHelperAsync() {
                var result = false;
                foreach (var processId in this.ProcessIds) result |= await ProcessSignaler.TrySignalAsync(processId, signal).ConfigureAwait(false);
                return result;
            }
        }
        ...
        /// <summary>
        /// A convenience method for calling <see cref="Shell.TryAttachToProcess(int,System.Action{Scriptable.Shell.Shell.Options}?,out Scriptable.Shell.Command?)"/> on <see cref="Shell.Default"/>
        /// </summary>
        public static bool TryAttachToProcess(int processId, Action<Shell.Options> options, [NotNullWhen(true)] out Command? attachedCommand) {
            return Shell.Default.TryAttachToProcess(processId, options, out attachedCommand);
        }

        /// <summary>
        /// A convenience method for calling <see cref="Shell.TryAttachToProcess(int,out Scriptable.Shell.Command?)"/> on <see cref="Shell.Default"/>
        /// </summary>
        public static bool TryAttachToProcess(int processId, [NotNullWhen(true)] out Command? attachedCommand) {
        ...
        void IDisposable.Dispose() {
            if (Interlocked.Exchange(ref this._disposed, 1) == 0) this.DisposeInternal();
        }
